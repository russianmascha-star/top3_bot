...
# ==================== Получение HTML через ScrapingBee ====================
def fetch_html_via_scrapingbee():
    try:
        ...
        logger.info(f"Запрос к ScrapingBee (с JS, ожиданием селектора и куками) для {TARGET_URL}")
...
def parse_draw_from_html(html):
    ...
    try:
            ...
            if script.string and ('__INITIAL_STATE__' in script.string or 'draws' in script.string):
                ...
                if match:
                    try:
                        ...
                        # Пытаемся найти draws в разных местах
                        draws = None
                        if 'draws' in data:
                            draws = data['draws']
                            ...
                            for key, value in data.items():
                                if isinstance(value, list) and len(value) > 0 and isinstance(value[0], dict) and 'number' in value[0]:
                                    draws = value
                                    break
        ...
        # Если не нашли JSON, парсим HTML-элементы
        ...
        if not draw_items:
            ...
            logger.debug(f"HTML фрагмент: {html[:2000]}")
            return None
